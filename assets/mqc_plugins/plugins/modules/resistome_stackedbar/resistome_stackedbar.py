...
class MultiqcModule(BaseMultiqcModule):
    def __init__(self):
        ...
        super(MultiqcModule, self).__init__(
            name="AMR Plus Plus Resistome Results",
            anchor="resistome_stackedbar"
        )
        self.comp_data = dict()
        taxonomy_level = OrderedDict({
            "perc_AMR_analytic_matrix.csv":"AMR_analytic_matrix",
        })
        ...
        # Read input files
        for f in self.find_log_files("resistome_stackedbar", filehandles=True):
            if f['fn'] in data_dict:
                df = pd.read_csv(f['f'], index_col=0)
                data_dict[f['fn']] = df
            else:
                log.warning("{} not recogized by resistome stackedbar module".format(f['fn']))
        ...
        for fn, df in data_dict.items():
            ...
            for minval in min_values:
                ...
                self.write_data_file(data, 'resistome_stackedbar')
                if len(data) > 0:
                ...
                config = {
                'id': 'Resistome_stackedbar' + level_interface_id,
                'use_legend': True,
