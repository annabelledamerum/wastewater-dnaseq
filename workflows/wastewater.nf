...
if (params.single_end && !params.skip_spades) { exit 1, "ERROR: spades --meta only accepts paired end or interleaved reads. Please specify --skip_spades = true" }
if (params.spades_mode == '' && !params.reference) { exit 1, "ERROR: spades running in isolate mode, but no reference genome provided for QUAST assembly QC. Please specify --reference" }
