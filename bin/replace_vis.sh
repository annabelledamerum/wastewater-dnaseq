...
do
    ...
    if [ $EMPTYFILE -lt 1 ]; then
        ...
        sed -i "5i parent_description: '<a href=\"https://pubmed.ncbi.nlm.nih.gov/26028277/\">ANCOM-BC</a> is a software designed to find differentially expressed taxa between user specified groups. By default, the first alphabetical group is used as the reference in the below plots. Positive log fold change values (<b>enriched</b> in the legend) indicate that the taxa is expressed more commonly in the subject group as opposed to the reference group. Negative log fold change values (<b>depleted</b> in the legend) indicate that the taxa is expressed more commonly in the reference group as opposed to the subject group.'" "${PREFIX}_mqc.html"
