...
def metaphlan_profileparse( mpa_profiletable, label):
    ...
    relprofile.columns = ["clade_name",label]
    relprofile = relprofile[relprofile["clade_name"].str.contains("s__") == True]
    relprofile = relprofile[relprofile["clade_name"].str.contains("t__") == False]
    relprofile.to_csv((label+"_relabun_parsed_mpaprofile.txt"), sep="\t", index=False)
     
    absprofile = profile[["clade_name", "estimated_number_of_reads_from_the_clade"]]
    absprofile.columns = ["clade_name",label]
    absprofile = absprofile[absprofile["clade_name"].str.contains("s__") == True]
    absprofile = absprofile[absprofile["clade_name"].str.contains("t__") == False]
    absprofile.to_csv((label+"_absabun_parsed_mpaprofile.txt"), sep="\t", index=False)
