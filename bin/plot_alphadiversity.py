...
vector = sys.argv[1]
def alpha_diversity_plot(shannon, faith_pd, evenness, observed_features, output_file):
    ...
    shannon=shannon.set_axis([*shannon.columns[:-1], 'Value'], axis=1, inplace=False)
    shannon ['Value'] = shannon['Value'].apply(pd.to_numeric)

    #prepare dataframe faith_pd
    faith_pd = pd.read_csv(faith_pd, sep = '\t')
    faith_pd = faith_pd.drop([0])
    faith_pd=faith_pd.drop(columns=['id'])
    faith_pd=faith_pd.set_axis([*faith_pd.columns[:-1], 'Value'], axis=1, inplace=False)
    faith_pd ['Value'] = faith_pd['Value'].apply(pd.to_numeric)

    #prepare dataframe evenness
    evenness = pd.read_csv(evenness, sep = '\t')
    evenness = evenness.drop([0])
    evenness=evenness.drop(columns=['id'])
    evenness=evenness.set_axis([*evenness.columns[:-1], 'Value'], axis=1, inplace=False)
    ...
    observed_features=observed_features.set_axis([*observed_features.columns[:-1], 'Value'], axis=1, inplace=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--faith_pd", "-f", type=str, help="Input  file")
    ...
    alpha_diversity_plot(args.shannon, args.faith_pd, args.evenness, args.observed_features, args.output_file)
