...
[MetaPhlAn4](https://github.com/biobakery/metaphlan) is a computational tool for profiling the composition of microbial communities (Bacteria, Archaea and Eukaryotes) from metagenomic shotgun sequencing data (i.e. not 16S) with species-level resolution via marker genes.
...
- `metaphlan4/`
  ...
  - `<db_name>/`
    ...
    - `<sample_id>.bowtie2out.txt`: BowTie2 alignment information (can be re-used for skipping alignment when re-running MetaPhlAn4 with different parameters)
    - `<sample_id>_profile.txt`: MetaPhlAn4 taxonomic profile including abundance estimates

</details>

The main taxonomic profiling file from MetaPhlAn4 is the `*_profile.txt` file. This provides the abundance estimates from MetaPhlAn4 however does not include raw counts by default.
...
- MetaPhlAn4: `<sample_id>_profile.txt` Taxpasta uses the `relative_abundance` column multiplied with a fixed number to yield an integer for the standardised profile.
